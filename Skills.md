...
> *   **Automatisk och proaktiv driftsättning (Deploy) & Git Master-flöde:** Git är den primära källan (master) för källkoden, och Linuxservern är sekundär driftsättningsmiljö (nummer två). Före påbörjat arbete MÅSTE Antigravity alltid synka sin lokala kodbas genom att köra `git pull` för att hålla sig synkad med den senaste koden. Så fort Antigravity gör några källkods- eller databasschemaändringar lokalt i arbetsytan, MÅSTE Antigravity automatiskt köra driftsättningsskriptet `py deploy.py`. Detta skript kommer först automatiskt committa och pusha alla ändringar till Git-repot (origin master), och därefter paketera och driftsätta koden till Linuxservern (`192.168.19.13`) samt synkronisera databasschemat. Driftsättningen ska ske helt proaktivt av Antigravity. Användaren ska aldrig behöva be om eller köra en manuell deploy, synk eller kontroll av databasen.
